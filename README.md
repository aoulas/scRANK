...
These will be installed automatically with scRANK with the exception of "riverplot" which is no longer available via cran and needs to be downloaded from archives using the link [here](https://cran.r-project.org/src/contrib/Archive/riverplot/riverplot_0.10.tar.gz)
and then installed using: install.packages(path_to_file, repos = NULL, type="source")\
